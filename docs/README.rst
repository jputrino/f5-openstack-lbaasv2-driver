...
It watches the Neutron RPC messaging queue for calls to the `Neutron LBaaS API`_ and schedules tasks to the `F5 OpenStack BIG-IP Controller`_.
...
.. important::
   ...
   * You must download and install the `F5 OpenStack BIG-IP Controller`_ and the :ref:`F5 Service Provider Package <f5-sp-package-install>` **before** you install the F5 LBaaSv2 driver.
...
.. seealso::
   ...
   `F5 OpenStack LBaaSv2 user documentation`_
...
.. _available F5 agent: somelink/for/agent/availability
...
.. _Configure and start: /products/openstack/openstack-bigip-ctlr/latest/index.html#configuration-parameters
